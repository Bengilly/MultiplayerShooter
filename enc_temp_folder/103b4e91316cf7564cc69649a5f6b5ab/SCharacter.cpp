...
void ASCharacter::ReloadWeapon(ASWeapon* EquippedWeapon)
{
	...
	//if (EquippedWeapon == WeaponClassArray[0])
	//{
	...
	//	{
	//		//if player doesn't have enough ammo to refill magazine, add remaining bullets to current mag
	//		EquippedWeapon->Reload(PlayerRifleAmmo);
	//		PlayerRifleAmmo = 0;
	//	}
	//}
	//else if (EquippedWeapon == WeaponClassArray[1])
	//{
	//	if (PlayerPistolAmmo >= AmmoToReload)
	//	{
	//		PlayerPistolAmmo -= AmmoToReload;
	//		EquippedWeapon->Reload(AmmoToReload);

	//		GetWorldTimerManager().ClearTimer(Timerhandle_Reload);
	//		UGameplayStatics::PlaySoundAtLocation(this, EndReloadSound, this->GetActorLocation());
	//		UE_LOG(LogTemp, Log, TEXT("Remaining Player Ammo: %s"), *FString::SanitizeFloat(PlayerPistolAmmo));
	//	}
	//	else if (PlayerPistolAmmo < AmmoToReload)
	//	{
	//		//if player doesn't have enough ammo to refill magazine, add remaining bullets to current mag
	//		EquippedWeapon->Reload(PlayerPistolAmmo);
	//		PlayerPistolAmmo = 0;
	//	}
	//}
